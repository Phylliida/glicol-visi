...
const ensureSettings = (node) => {
    ...
    if (node.settings.text === undefined) node.settings.text = '';
    if (node.value === undefined) node.value = node.settings.text ?? '';
};

const getOutputValue = (node) => node?.settings?.text ?? node?.value ?? '';
